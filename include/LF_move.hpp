...
// Row of the LF table (bitpacked)
class LF_row
{
public:
    uchar character;
    ulint length : BYTES_TO_BITS(LENGTH_BYTES);
    ulint pointer : BYTES_TO_BITS(POINTER_BYTES);
    ulint offset : BYTES_TO_BITS(OFFSET_BYTES);

    LF_row() {}
    LF_row(uchar c, ulint l, ulint p, ulint o) : character(c), length(l), pointer(p), offset(o) {}
} __attribute__((packed));

// Row of the LF table (unpacked)
class LF_row_raw
{
public:
    uchar character;
    ...
    LF_row_raw() {}
    LF_row_raw(uchar c, ulint l, ulint p, ulint o) : character(c), length(l), pointer(p), offset(o) {}
};
...
public:
    ...
    {
        assert(p.run < LF_runs.size());
        return LF_runs[p.run];
    }
    ...
    {
        position next_pos = {get(pos).pointer, get(pos).offset + pos.offset};
    ...
    std::optional<position> pred_char(position pos, uchar c)
    {
        ...
        pos.offset = get(pos.run).length - 1;
    ...
    std::optional<position> succ_char(position pos, uchar c)
    {
        while (get_char(pos) != c)  
        {
            if (pos.run == r - 1) return std::nullopt;
            ++pos.run;
        }
        pos.offset = 0;
